...
export default function CreateCommentsInComment({ user, commentId , setComments, setCount  }) {
  ...
  const handleComment = async (e) => {
    if (e.key === "Enter") {
      if (commentImage != "") {
        ...
        const path = `${user.username}/comment_images/${commentId}`;
        ...
        const comments = await commentInComment(commentId, text, "", user.token);
        setComments(comments);
        setCount((prev) => ++prev);
        setLoading(false);
        setText("");
